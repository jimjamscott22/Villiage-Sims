...
- Use the browser dev server for visual verification: `npm run dev` serves at
  ...
  deterministic **browser-demo** transport that reproduces the same terrain + orbiting villager,
  so the full render/interpolation pipeline is testable without a desktop webview.
